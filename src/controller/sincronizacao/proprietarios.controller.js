...
const { validacaoDosProprietarios } = require('../../utils/validacao.dados');
...
class Proprietarios extends GerarArquivo {
      ...
      async fnStart(value) {
            ...
            data.query_sql = value ? ProprietarioSetData(moment(value, ['DD/MM/YYY HH:mm', "YYYY/MM/DD HH:mm"]).format("YYYY/MM/DD HH:mm")) : ProprietarioGetAll()
      ...
      async sincronizar(data) {
            return new Promise(async (resolve) => {
                  ...
                  if (arrayDados?.length > 0) {
                        ...
                        await this.fnGeradorArquivos(dataEncriptado, 'SYNCz_PROPRIETARIOS', this.pastaSync)

                        dadosLogs = {
                              data: moment().format("YYYY/MM/DD HH:mm:ss"),
                              qtd: arrayDados?.length || 0,
                              tipo: 'SYNCz_PROPRIETARIOS',
                              count: moment().format("YYYY/MM/DD HH:mm:ss").valueOf()
                        }
                        await criarLogSincronizacao(dadosLogs)
                  }

                  console.log(`[PROPRIETARIOS X] || ${dadosLogs?.qtd || 0}`);
                  resolve(true)
