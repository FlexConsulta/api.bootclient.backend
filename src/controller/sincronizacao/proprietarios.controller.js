...
const { ProprietarioGetAll, ProprietarioSetData } = require('../../config/sql/proprietarios');
...
class Proprietarios extends GerarArquivo {
      ...
      async sincronizar() {
            return new Promise(async (resolve) => {
                  ...
                  let __resultCount = await resultadoSequelize.obterDados(this.empresa.sql_count_proprietarios);
                  ...
                  if (__resultCount > SQL_LIMIT) {
                        ...
                        await this.fnGeradorArquivos(dataEncriptado, 'SYNCz_PROPRIETARIOS', this.empresa.cnpj_empresa, FOLDER_SYNC_SUCCESS)

                        const dadosLogs = {
                          cnpj_cliente: this.empresa.cnpj_empresa,
                          error: false,
                          nome_arquivo: `SYNCz_PROPRIETARIOS_${this.empresa.cnpj_empresa}`,
                    ...
                    const resultadoSequelize = await new sequelizePostgres(dataConexao)
                    const data = moment(this.empresa.dt_ultima_sync_proprietarios, ["DD/MM/YYY HH:mm","YYYY/MM/DD HH:mm"]).format("YYYY/MM/DD HH:mm")
                    // ALTERAR SQL - COLOCAR DATA
                    const arrayDados = await resultadoSequelize.obterDados(this.empresa.sql_proprietarios);
                    const dataEncriptado = await encryptedData(arrayDados)
                    await this.fnGeradorArquivos(dataEncriptado, 'SYNCz_PROPRIETARIOS', this.empresa.cnpj_empresa, FOLDER_SYNC_SUCCESS)
