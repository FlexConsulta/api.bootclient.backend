...
class Viagens extends GerarArquivo {
      ...
      async sincronizar() {
            return new Promise(async (resolve) => {
                  ...
                  let __resultCount = await resultadoSequelize.obterDados(this.empresa.sql_count_viagens);
                  ...
                  if (__resultCount > SQL_LIMIT) {
                        ...
                        const _sql = `${this.empresa.sql_viagens} LIMIT ${SQL_LIMIT} OFFSET ${offset}`;
                        const resultadoSequelize = await new sequelizePostgres(dataConexao)
                        const arrayDados = await resultadoSequelize.obterDados(_sql);
                        const dataEncriptado = await encryptedData(arrayDados)
                        await this.fnGeradorArquivos(dataEncriptado, 'SYNCz_VIAGENS', this.empresa.cnpj_empresa, FOLDER_SYNC_SUCCESS)

                        const dadosLogs = {
                          cnpj_cliente: this.empresa.cnpj_empresa,
                          error: false,
                          nome_arquivo: `SYNCz_VIAGENS_${this.empresa.cnpj_empresa}`,
                          entidade: "viagens",
                        };
                    ...
                    const resultadoSequelize = await new sequelizePostgres(dataConexao)
                    const dataLastViagen = moment(this.empresa.dt_ultima_sync_viagens, ["DD/MM/YYY HH:mm","YYYY/MM/DD HH:mm"]).format("YYYY/MM/DD HH:mm")
                    // ALTERAR SQL - COLOCAR DATA
                    const arrayDados = await resultadoSequelize.obterDados(this.empresa.sql_viagens);
                    const dataEncriptado = await encryptedData(arrayDados)
                    await this.fnGeradorArquivos(dataEncriptado, 'SYNCz_VIAGENS', this.empresa.cnpj_empresa, FOLDER_SYNC_SUCCESS)
